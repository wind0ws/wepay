...
/**
 * From https://github.com/badoo/android-weak-handler/blob/master/src/main/java/com/badoo/mobile/util/WeakHandler.java
 * Credit to badoo.
 *
 */
class WeakHandler{
    private var mCallback: Handler.Callback? = null // hard reference to Callback. We need to keep callback in memory
    private var mExec: ExecHandler = ExecHandler()
    ...
    fun post(runnable:()->Unit):Boolean = post(Runnable { runnable.invoke() })
     ...
     */
    fun postAtTime(uptimeMillis: Long,r: Runnable): Boolean {
        return mExec.postAtTime(wrapRunnable(r), uptimeMillis)
    }

    fun postAtTime(uptimeMillis: Long,r:()->Unit) = postAtTime(uptimeMillis, Runnable { r.invoke() })
     ...
     */
    fun postAtTime( uptimeMillis: Long,token: Any,r: Runnable): Boolean {
        return mExec.postAtTime(wrapRunnable(r), token, uptimeMillis)
    }

    fun postAtTime(uptimeMillis: Long,token: Any,r: ()->Unit) = postAtTime(uptimeMillis, token, Runnable { r.invoke() })
    ...
    internal class ChainedRef(var lock: Lock, val runnable: Runnable?) {
        ...
        val wrapper: WeakRunnable = WeakRunnable(if (runnable!=null) WeakReference(runnable) else null, WeakReference(this))
